...
public class TestsTransaction extends junit.framework.TestCase {
    ...
    public void testDesMementos() {

        GroupeDeContributeurs neveux = new GroupeDeContributeurs("Neveux");
        neveux.ajouter(new Contributeur("Riri", 100));
        neveux.ajouter(new Contributeur("Fifi", 200));
        neveux.ajouter(new Contributeur("Loulou", 50));

//        VisiteurSauvegarde vs = new VisiteurSauvegarde();
//        Memento m = new Memento(neveux);

        GroupeDeContributeurs superRiches = new GroupeDeContributeurs("SuperRiches");
        superRiches.ajouter(new Contributeur("Oncle Picsou", 300)); // manquerai plus que Oncle Picsou dépasse Integer.MAXVALUE;

        GroupeDeContributeurs donaldVille = new GroupeDeContributeurs("DonaldVille");

        donaldVille.ajouter(superRiches);
        donaldVille.ajouter(neveux);

        Gardien gardien = new Gardien();

        gardien.setMemento(new Memento(donaldVille)); ;

        TransactionDebit t = new TransactionDebit(donaldVille);

        try {
            t.debit(20);
        } catch (Exception e) {
            gardien.getMemento().setState(donaldVille);
        }

        VisiteurToString vts = new VisiteurToString();

//        System.out.println(donaldVille.accepter(vts));

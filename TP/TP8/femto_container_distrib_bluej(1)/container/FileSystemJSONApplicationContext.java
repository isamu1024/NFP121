...
public class FileSystemJSONApplicationContext extends AbstractApplicationContext{
    ...
    public FileSystemJSONApplicationContext(){
        super();
    }
                    ...
                    try {
                        ...
                        try {
                            propertyId = (config.getJsonObject(rangBean).getJsonArray("property."
                                    + indexProperty+".param.1")).toString();
    ...
    private Object parseArray(Class<?> cl, String str) throws Exception{
        ...
        if( elementClass == Object.class) {
            try {
                ...
                try {
                    for(int i=0;i<t.length;i++) {
                        double j=Double.parseDouble(t[i]);
                    }
                    elementClass=Double.class;
                }catch (Exception e2){
                    try {
                        for(int i=0;i<t.length;i++) {
                            if(!(t[i].equals("true") || t[i].equals("false")) )
                                throw new Exception();
                        }
                        ...
                        try {
                            for(int i=0;i<t.length;i++) {
                                if(t[i].length() != 3)      // " ouvrant et " fermant compris
                                    throw new Exception();
                            }
    ...
    static{
        try{
            ...
            map.put(char.class, FileSystemJSONApplicationContext.class.
                getDeclaredMethod("parseChar",String.class));
            map.put(Character.class, FileSystemJSONApplicationContext.class.
                getDeclaredMethod("parseChar",String.class));
            map.put(String.class, FileSystemJSONApplicationContext.class.
                getDeclaredMethod("parseString",String.class));

            // Cas qui n'a pas de sens, retourne une String par défaut
            map.put(Object.class, FileSystemJSONApplicationContext.class.

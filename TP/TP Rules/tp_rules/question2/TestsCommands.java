...
public class TestsCommands extends junit.framework.TestCase {
    ...
    public void testMacroCommandException() throws Exception {
        ...
        MacroCommandException<String, Integer> moins4 = new MacroCommandException<>();
    ...
    public void testMacroReflectivite() throws Exception {

        Class<?> macroClass = null;
        CommandI<Integer, Integer> macro = null;

        try {
            macroClass = Class.forName("question2.MacroCommand");
        ...
        for (Method m : macroClass.getMethods()) {
            if (m.getName().equals("add"))
                add = m;
        }

        Class<?> decClass = Class.forName("question2.TestsCommands$Dec");
    ...
    public void testInvocationError() throws Exception {
        ReflectCommand test = new ReflectCommand();
        test.setCommand("question2.TestsCommands$Dec");
        MacroCommand<String, Integer> moins2 = new MacroCommand<>();

        moins2.add(test).add(test);

        try {
            int result = moins2.execute("", 1);
            fail("doit lever une exception");
        } catch (Exception exc) {
            assertTrue(exc instanceof InvocationTargetException);
        }
    }
